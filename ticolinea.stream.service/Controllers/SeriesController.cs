...
namespace ticolinea.stream.service.Controllers
{
    ...
    public class SeriesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Obtener(string usuario, string password)
        {
            ...
            using (Mariadb mariadb = new(Constantes.Global.MARIADB_CONN))
            {
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                    while (reader.Read())
                    {
                        series.Add(new Modelos.Serie
                        {
                            ...
                            URL = $"http://tv.play-latino.com:27701/Peliculas/Reproducir/{reader.GetInt32(6)}/test/test.{reader.GetString(9)}",
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                    while (reader.Read())
                    {
                        episodioRequest.Episodios.Add(new Modelos.Episodio
                        {
                            ...
                            URL= $"http://tv.play-latino.com:27701/Peliculas/Reproducir/{reader.GetInt32(8)}/test/test.{reader.GetString(5)}"
                        });
                    }
            }
